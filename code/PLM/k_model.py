...
class Attention(nn.Module):
    def __init__(self, args: ModelConfig):
        super().__init__()
        # 根据是否指定n_kv_heads确定用于键值对的头数
        self.n_kv_heads = args.n_heads if args.n_kv_heads is None else args.n_kv_heads
        # 确保总头数可以被n_kv_heads整除
        ...
        # 定义dropout层，可以试着理解为啥现在的大模型基本很少使用dropout
        ...
        if not self.flash:
            warnings.warn("Flash Attention is not available. Using standard attention.")
            # 创建一个上三角矩阵，用于遮蔽未来的信息
            mask = torch.fill((1,1,args.max_seq_len), float('-inf'))
...
if __name__ == '__main__':
    args = ModelConfig()
    mlp = MLP(dim = args.dim*4,hidden_dim = args.dim*4, multiple_of=args.multiple_of, dropout=args.dropout)
    # 模拟输入数据
    x = torch.randn(1, 50, args.dim*4)
    # 运行MLP模型
    output = mlp(x)
    print(output.shape)
'''
...
if __name__ == '__main__':
    args = ModelConfig()
    decoder_layer = DecoderLayer(layer_id=0, args=args)
    freqs_cos, freqs_sin = precompute_freqs_cis(args.dim//args.n_heads, 50)
    # 模拟输入数据
    x = torch.randn(1, 50, args.dim)
    # 运行EncoderLayer模型
    output = decoder_layer(x, freqs_cos, freqs_sin)
    print(output.shape)
'''
